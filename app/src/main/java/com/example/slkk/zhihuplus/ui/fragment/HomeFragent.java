...
//        listView = (ListView) View.inflate(getActivity(), R.layout.listview,null);
    ...
    protected Object getData() {
        //如果是下拉刷新则清空集合数据
        if(refreshListView.getCurrentMode()== PullToRefreshBase.Mode.PULL_FROM_START) list.clear();

        final Home home = DataLoader.getInstance().loadBeanData(Url.Home+list.size(), Home.class);
        if(home!=null){
